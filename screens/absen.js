...
import Toast from 'react-native-toast'; // or import { Toast } from 'react-native-simple-toast';
...
export const MemoizedMapComponent = React.memo(({ latitude, longitude, address, userLocation, selectedLocation }) => {
  ...
  const destination = useMemo(() => {
    if (selectedLocation === 'Kantor Utama') {
      return { latitude: -7.322275666311146, longitude: 112.65087023369988 };
    } else if (selectedLocation === 'Kantor Cabang') {
      return { latitude: -7.314289595990596, longitude: 112.7057918038738 };
    } else {
      return null;
    }
  }, [selectedLocation]);
...
const Absen = () => {
  ...
  const handleAbsenPulang = async () => {
    try {
      ...
      Toast.show('Berhasil Absen Pulang', Toast.SHORT);
    } catch (error) {
      console.error('Error:', error);
      Toast.show('Gagal Absen Pulang', Toast.SHORT);
    }
  };
    ...
    <>
      <ScrollView backgroundColor={'white'}>
        <MemoizedMapComponent
          ...
          longitude={
            selectedLocation === 'Kantor Utama' ? 112.739756 : selectedLocation === 'Kantor Cabang' ? 112.739756 : 112.739756
          }
        ...
        <Box style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: 'rgba(0, 0, 0, 0.5)' }}>
          <Box backgroundColor="white" p={4} borderRadius={10} width={300} >
            ...
            <Box flexDirection="row" m={2}>
              ...
              <Box ml={2}>
                {/* Tombol Tidak untuk konfirmasi absen */}
